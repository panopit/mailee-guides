...
Templates criados para o *Mailee.me* precisam obedecer certas regras. São elas:

* Todo conteúdo salvo em codificação UTF-8
* Arquivo _zip_ contendo
...
h4. Áreas editáveis em templates do *Mailee.me*
...
Para tornar essa <samp>&lt;div&gt;</samp> em um bloco de repetição de notícias, basta adicionar um atributo de <samp>class</samp>, um de <samp>id</samp> e outro de <samp>title</samp>, assim:
...
Dessa forma, o *Mailee.me* interpretará tudo que estiver dentro dessa <samp>&lt;div id="news" class="repeat" title="notícias"&gt;</samp> como um bloco de repetição.

INFO. Dentro dos blocos de repetição não será necessário definir <samp>class="edit"</samp> nos elementos, pois o bloco de repetição, por padrão, já é editável.
...
Nesse caso, a informação do atributo <samp>id=""</samp> é <samp>"news"</samp>.
...
Dessa forma, o *Mailee.me* irá interpretar o elemento <samp>&lt;ul class="index news"&gt;</samp> como sendo a lista de índice para o bloco de repetição de notícias. No final seu código deverá conter informações similares às apresentadas abaixo:
...
O *Mailee.me* gera *links dinâmicos* facilitando as tarefas comuns em email marketing.

Você pode adicionar facilmente um *link para descadastramento (unsubscribe)*, atendendo uma funcionalidade básica para boas práticas de email marketing, um *link para visualização na web*, para usuários que tenham dificuldade em visualizar a mensagem em seu cliente de email, um *link para encaminhar a mensagem (forward)*, fornecendo ao usuário a possibilidade de enviar suas mensagens para amigos e um *link para adicionar à lista de contatos (whitelist)*, que levará a uma página com os passos necessários para o usuário adicionar o seu email na lista de contatos.

h4. Link para descadastramento (unsubscribe)

O *link de descadastramento* ou *unsubscribe* é um item recomendado a todas mensagens de email marketing. Ele serve para o usuário que quiser deixar de receber suas notícias por email, seja por desinteresse, ou qualquer outro motivo, possa ser efetivamente descadastrado de sua lista.

O *Mailee.me* oferece uma solução simples, e, para inserir esse link em seus templates, basta apenas colocar esse código em seus templates:
...
INFO. Você também poderá utilizar um atributo <samp>class="unsubscribe"</samp> ou <samp>id="unsubscribe"</samp>, porém, lembre-se que ao usar o atributo <samp>id="unsubscribe"</samp> ele só poderá ser utilizado UMA vez por template, e, portanto, não é indicado caso você precise colocar mais de um link *unsubscribe* em seu template.
...
h4. Informações de campos dinâmicos

O *Mailee.me* possui *campos dinâmicos* que podem ser usados para personalizar suas mensagens com os dados dos seus contatos.
...
Você *pode deixar vazio* o conteúdo da <samp>&lt;span class="contact name"&gt;&lt;/span&gt;</samp>, e o *Mailee.me* irá inserir automaticamente a informação contida na <samp>tag</samp> <samp>"name"</samp> do contato de sua lista, e, como _fallback_ exibirá o texto *"Bom dia! Confira as ofertas selecionadas para você."*.
...
Confira a lista completa de *campos dinâmicos*:
...
"*Campos personalizados*":contacts_custom_fields.html podem ser criados na importações de suas bases de dados ou diretamente no *Mailee.me*. Os dados importados podem também ser inseridos em seus templates ou utilizados para "*segmentação de listas*":segmentation.html.
...
Enquanto *Nome* e *E-mail* são *campos dinâmicos* no *Mailee.me*, o campo *Gosta de* é personalizado. Ele será importado e você poderá associar a uma coluna da seguinte forma:

!images/templates/importing_fields.gif(Campos personalizados importados)!

Chamaremos esse campo personalizado de *estilo* e para inserir o campo em seu template, basta colocar no seu template o seguinte código:
...
INFO. *Importante:* <br/> Para os *campos dinâmicos* você deve usar a <samp>tag</samp> deste campo e não o *nome*.


h4. Informações da lista

Todas as mensagens enviadas pelo *Mailee.me* estão relacionadas à uma lista de contatos. Para colocar os *dados da lista* basta colocar o atributo <samp>class="list"</samp> seguida do campo que você desejar.
...
Para inserir a informação de *endereço* relacionada a lista, basta adicionar um <samp>&lt;span&gt;</samp> com o atributo <samp>class="contact name"</samp>, ficando assim:
...
Veja como cadastrar essas informações em "Listas de contatos":contacts_lists.html.
